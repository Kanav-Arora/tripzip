...
export default function SectionA() {
    ...
    const constants = {
        ...
        description: "Europe's enchanting beauty is an unrivaled blend of natural beauty and cultural richness",
    }

    return (
        <SectionLayout subheading={constants.subheading} title={constants.title} description={constants.description}>
            <div className='flex flex-col gap-y-4 z-0'>
                <div className='flex flex-row gap-x-4'>
                    <div class="w-3/5 h-[250px] shadow-xl relative overflow-hidden rounded-2xl">
                        <img class="w-full" src={data[0].url} alt="Switzerland" />
                        <div class="absolute bottom-0 left-0 p-4 text-white font-bold">{data[0].place}</div>
                    </div>

                    <div class="w-2/5 h-[250px] shadow-xl relative overflow-hidden rounded-2xl">
                        <img class="w-full" src={data[1].url} alt="Greece" />
                        <div class="absolute bottom-0 left-0 p-4 text-white font-bold">{data[1].place}</div>
                    </div>
                </div>
                <div className='flex flex-row gap-x-4'>
                    <div class="w-2/5 h-[250px] shadow-xl relative overflow-hidden rounded-2xl">
                        <img class="w-full" src={data[2].url} alt="Switzerland" />
                        <div class="absolute bottom-0 left-0 p-4 text-white font-bold">{data[2].place}</div>
                    </div>

                    <div class="w-3/5 h-[250px] shadow-xl relative overflow-hidden rounded-2xl">
                        <img class="w-full" src={data[3].url} alt="Greece" />
                        <div class="absolute bottom-0 left-0 p-4 text-white font-bold">{data[3].place}</div>
                    </div>
                </div>
            </div>
        </SectionLayout>
